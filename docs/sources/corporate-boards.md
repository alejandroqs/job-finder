...
Purpose: describe Sagulpa, Guaguas Municipales, GEURSA, GSC, Aena, Indra Group, FULP, and SODETEGC web monitoring, active-window rules, and attachment handling.

Read when: changing a web-board parser, HTML selector, closing-date rule, title rejection, or temporary PDF flow.

Source of truth: the Sagulpa, Guaguas, GEURSA, GSC, Aena, Indra, FULP, and SODETEGC fetcher/parser modules under [`src/job_finder`](../../src/job_finder/), [`keyword_filter.py`](../../src/job_finder/), and the corresponding tests under [`tests`](../../tests/).

## Shared pattern

The web-board interfaces separate list retrieval from detail retrieval. Parsers identify active entries, fetch or receive detail HTML, extract text, and convert the result to `BOPage`. A failing detail or attachment operation is handled inside the parser so one listing does not necessarily terminate a source scan. GEURSA and GSC are intentionally list-only and do not fetch a detail page or PDF while parsing their lists. SODETEGC is a bounded status monitor rather than an offer parser: it does not create `BOPage` objects or enter keyword filtering, and its changed-section result is represented as a typed source notice.
...
`SODETEGCFetcher` reads only the fixed public employment page at [`sodetegc.org/conocenos/informacion-administrativa/empleo`](https://www.sodetegc.org/conocenos/informacion-administrativa/empleo/). Requests use explicit connect/read timeouts, no automatic retries, at most two manually validated redirects to the same official page path on an HTTPS official-host variant, and a 2 MiB response cap. The fetcher does not follow page links, historical entries, PDFs, or application routes.

`SODETEGCParser` verifies page identity and the owned page/section structure before comparing visible text. It bounds `Convocatorias abiertas` at the next owned heading of equal or higher rank, or the verified owner boundary, retaining lower-level content while excluding hidden content, navigation, scripts, and nested foreign owners. It compares the whole section against “Actualmente no hay ninguna convocatoria abierta” after Unicode, case, whitespace, and NBSP normalization and tolerates an optional final full stop. Forecast, history, footer, attributes, styling, and link destinations do not determine the result. A recognized empty section with a verified subsequent boundary is changed; missing or ambiguous structure and invalid identity are unverifiable.

The parser returns `BASELINE`, `CHANGED`, or `UNVERIFIABLE`, with owned text and diagnostics. Only `CHANGED` creates a typed source notice; it bypasses the keyword filter and Gemini, links to the official page, and asks for manual review without asserting a vacancy or eligibility. The fixed-reference comparison runs on every scan: every successful scan while the section differs emits a notice, and a later baseline response is silent. No previous-run state or deduplication is stored. `target_date` does not reconstruct historical page state. The observed fixture and capture details are recorded in [`sodetegc_empleo_observed.md`](../../tests/fixtures/sodetegc_empleo_observed.md); future page markup remains unverified.

## Operational caveats

All eight portals are external SSR websites whose markup and availability can change. The local tests use fixtures and injected mock fetchers. They establish parser behaviour for covered HTML, dates, and errors; they do not prove that any live portal remains reachable or unchanged. The GSC implementation does not add pagination or load-more retrieval; the inspected snapshot had no pagination controls, and a generic `infinitescroll.min.js` include is not evidence of additional records. A historical GSC target date filters the current page response and does not reconstruct historical website state. Repeated scans within the same inferred window can produce repeated findings or notifications because no persistent cross-run deduplication is introduced. FULP likewise has no persistent cross-run deduplication, and its current-page discovery, detail failures, 200-attempt/180-second scheduling guard, and source markup are not deployment guarantees. The implementation's bounded read-only FULP check on 2026-09-16 used one public listing response and six distinct detail URLs; it did not follow private/application links, invoke AI, send notifications, or deploy. Indra has no persistent cross-run deduplication either: IDs are deduplicated within one scan before detail fetching and AI only. SODETEGC is a fixed-reference status check whose notices repeat on each successful differing scan. Its observed fixture captures one page response, not future markup or delivery behaviour. A 2026-09-15 read-only Indra discovery check reached the portal but was incomplete after 31 pages, 763 genuine rows, 733 unique IDs, contradictory totals, and a repeated-page diagnostic. That observation does not establish that the portal itself repeated the page: client-side navigation and identity handling can generate the diagnostic, so the historical counts remain observations rather than a causal explanation. A separate bounded discovery-only check after the repair parsed 30 pages, discovered 732 unique IDs, and reported complete coverage at that moment; it did not fetch the detail pages. It sampled three detail pages and made no AI calls. These are source-runtime observations, not a deployment or Lambda validation.
