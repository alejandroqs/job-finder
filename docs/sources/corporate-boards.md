...
Listing extraction is bounded to `.panel_ofertas` and `.row.oferta`; detail extraction is bounded to the owning `.Content-Oferta`. It uses owned `h3`/`h2.titulo`, `h5`, `.container-details`, `.Descripcion`, `.Tareas`, `.Perfil`, and `.Requisitos` content, excludes controls/sharing/footer/policy boilerplate, preserves unlabeled summary values, and uses the neutral employer value `Empresa no identificada` when no employer is published. Detail identity is taken from a validated canonical or `og:url` value and cross-checked against the requested/listing/response numeric ID. Offline details without that identity are quarantined; offline listing snapshots are explicitly listing-evidence-only and do not fetch details.

`target_date` is used only to evaluate explicit application-deadline evidence in the current HTML. Publication dates are metadata, not deadlines; closed, expired, conflicting, invalid, or ambiguous availability is not silently converted into an open offer. The normalized output is one whitespace-normalized paragraph per accepted detail, with role, eligibility, responsibilities, metadata, and raw evidence labels preserved for the shared `KeywordFilter` and optional existing Gemini stage. No geographic or personal-fit restriction is added and no persistent cross-run deduplication is introduced.
