...
The current source groups mean that a future deployment will make `ALL`/default Lambda scans and `ES` scans include GSC and Indra Group. `EU` and explicit selections of other individual sources do not include them. Indra's default blank catalogue traversal can issue one request per discovered search page plus one detail request per deduplicated listing; runtime suitability for the scheduled Lambda path is not established here. This source change does not alter the EventBridge schedule and no deployment or schedule operation is performed by the implementation task.
