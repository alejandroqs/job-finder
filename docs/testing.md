...
| `test_sodetegc_fetcher.py` | Fixed URL, timeout, redirect allowlist/hop limit, response size/content type/UTF-8 validation, and request failures. |
| `test_sodetegc_parser.py` | Observed section baseline, normalization, owned section boundaries, visibility/identity checks, changed and empty sections, and unverifiable structures. |
| `test_sodetegc_integration.py` | Online/offline parity, repeat-scan behavior, source selection, typed keyword/Gemini bypass, and mixed-scan ordering. |
| `test_source_notice_outputs.py` | Source-notice-only and mixed console/Markdown/HTML/Discord/Telegram/Lambda formatting with mocked notification calls. |
...
Fixtures are synthetic or local samples unless their provenance says otherwise. Indra fixture provenance and the distinction between observed layout fragments and synthetic edge cases are recorded in [`tests/fixtures/indra_provenance.md`](../tests/fixtures/indra_provenance.md); the bounded FULP observations and synthetic mutations are recorded in [`tests/fixtures/fulp_provenance.md`](../tests/fixtures/fulp_provenance.md); the observed SODETEGC employment page and capture provenance are recorded in [`tests/fixtures/sodetegc_empleo_observed.md`](../tests/fixtures/sodetegc_empleo_observed.md). Network calls are generally mocked. A passing suite therefore establishes deterministic parser and orchestration behaviour for the covered cases; it does not establish that official portals, Playwright, Gemini, Discord, Telegram, AWS, or the deployment workflow currently work in production. The SODETEGC output tests mock notifications and its AI tests use fakes; they establish neither actual delivery nor live AI classification.
